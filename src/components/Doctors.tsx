...
function ProviderCard({
  ...
  return (
    <div
      className={`shrink-0 rounded-2xl bg-neutral-200 p-2 ${className}`}
    >
      {/* Image */}
      ...
      <div className="px-1 pb-1 pt-2">
        <p className="truncate md:text-[10px] font-medium lg:text-sm text-primary">
          {provider.name}
        </p>

        <p className="truncate md:text-[10px] lg:text-xs text-secondary">
          {provider.specialty}
        </p>
...
function MoreCard({ className = "" }: { className?: string }) {
  return (
    <div
      className={`flex shrink-0 items-center justify-center rounded-2xl bg-emerald-50 p-6 text-center ${className}`}
    >
...
function DesktopGrid() {
  return (
    <div className="mx-auto hidden w-fit items-start gap-1 md:flex lg:gap-2">
      {DESKTOP_COLUMNS.map((column, columnIndex) => (
        <div
          key={columnIndex}
          className={`provider-column flex flex-col gap-2 ${
            column.shift ? "md:mt-20 lg:mt-20" : ""
          }`}
        >
            ...
            <ProviderCard
              ...
              provider={provider}
               className={`w-[100px] lg:w-[130px] ${
                provider.id === "morrison" || provider.id === "laurent"
                    ? "mt-40 lg:mt-50"
                    : ""
                }`}
            />
          ...
          {column.more && (
            <MoreCard className="w-[100px] lg:w-[130px] h-[200px] lg:h-[250px]" />
          )}
        </div>
      ))}
    </div>
...
function MobileCarousel() {
  ...
  useEffect(() => {
    ...
    const originalCount = PROVIDERS.length + 1;
    const gap = 16;
...
export default function VerifiedProviders() {
  return (
    <section className="w-full px-4 py-4 md:py-16 sm:px-6 lg:px-8 mb-16 lg:mb-32">
      {/* Heading */}
      <div className="mx-auto max-w-6xl text-center">
        <h2 className="heading">
          Meet Some Of Our Verified <br/>Providers
        </h2>
